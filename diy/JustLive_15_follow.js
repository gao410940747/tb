...
//    homeUrl: '/api/live/getRecommendByPlatformArea?platform=bilibili&area=舞见&page=1&size=1',//网站的首页链接,用于分类获取和推荐获取
    ...
    class_name: '全部&哔哩哔哩&抖音&斗鱼&虎牙&快手&网易CC',
    class_url: 'all&bilibili&douyin&douyu&huya&kuaishou&cc',
    filterable: 1,
    filter_url: '{{fl.area}}',
    filter: {
...
//                            var roomPic = pd(it, ".Xw5Zorxg img&&src");
    ...
    二级: `js:
        if (/快手/.test(input)) {
            ...
            var 高清 = '';
            var 标清 = '';
            info[4].split("；").map(function(it) {
                // 清晰度
                var qingxidu = it.split("，")[0];
                var play_info = qingxidu + "$" + it.split("，")[1] + "#";
                if(qingxidu === '默认') {
                    默认 = play_info;
                }
            ...
            let playFrom = [];
            let playList = [];
            playFrom.append('官方flv线路（若播放失败需切换EXO播放器）');
            playList.append(默认 + 蓝光 + 超清 + 高清 + 标清);
